...
- **CPU 核心调度（按启动阶段分别设置）**：为游戏启动的三个阶段各设一档 —— **Logo 阶段**（此阶段禁超线程可让 logo 秒进）、**窗口加载阶段**（加载界面/主菜单/大厅：部分机器禁小核可秒进，部分机器如被 Win11 自动优化过则需"全部核心(强制)"）、**游玩阶段**（禁超线程 / 禁 CPU0 缓解间歇性卡顿）。每档可选：不干涉（什么都不设置，含系统自动优化）/ 全部核心(强制) / 禁超线程 / 禁小核（仅 Intel 12代+ 大小核，其它平台不适用且不做改动）/ 及其组合（游玩档另有禁 CPU0 的组合）。另有全局"最大逻辑核心数"上限。除让引擎按当前阶段的核数自居外，还会真实设置游戏进程的亲和性（任务管理器「设置相关性」可见）。
...
cpu_cores=0             ; 全局上限：游戏可用最大逻辑核心数（0 = 不限制）
