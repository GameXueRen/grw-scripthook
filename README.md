...
| 插件改造：解锁游戏全部语言（AllLanguages，原名 LastRites_dlcfix） | 已完成，官方已修复闪退，改为解锁全部文本语言 |
...
[plugins]
                        ; 每个插件一行：1 = 加载；**没有这一行也 = 加载**（默认）；只有明确写 0 才是禁用
                        ; 扫描不会自动补行，所以这张表里只会有你（或菜单）真正改过的插件
                        ; 例：firstperson=0 表示关掉它；删掉这一行又等于加载
