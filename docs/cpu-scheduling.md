...
| `cpu_eco_boot` | Logo + 窗口加载（这两个阶段共用）| `0` 不干预（默认）· `1` 开：这两个阶段保持效率模式（**本机不支持时改为保持「低」优先级类**）· `2` 关：把这两个阶段的效率模式丢掉。`0` 与 `1` 沿用旧的开关语义，老配置读起来不变 |
...
| 阶段 | 从 | 到 | 判据 |
| --- | --- | --- | --- |
| **Logo** | 进程启动 | 游戏主窗口出现 | 两个窗口靠**两个特征**分辨：**窗口类**（启动画面 `ScimitarSplashScreenWindow`，主窗口 `ScimitarEngineWindowClass`，两边都按**全名精确匹配**，模块与覆盖层用的是同一对名字）与**标题**（启动画面里注册商标是乱码 `Ghost Recon?Wildlands`，主窗口正常 `Ghost Recon(R) Wildlands`）。**先看类名**（类名是结构性的，乱码只是编码意外），类名不认识时退回只看标题 —— 所以"主窗口还没起来"是精确的。**这一档不设期限**：主窗口等多久都等（小窗有时停留好几分钟），不会因为超时就把它跳过去 |
...
一个 **120 秒看门狗**只为"信号永远不来"兜底，而且是给**状态**那一个信号用的（状态机 hook 没装上的 build）：它把地板抬到下一档并记一行日志，并且只在状态机**从未读到过**时才触发 —— 能读到就说明信号是真的，阶段就一直等它，第一次加载再慢也等。

**窗口那一个信号没有看门狗，也没有期限**（曾经有过一个 120 秒的，2026-09-25 删除）：主窗口等多久都等，等不到就一直留在 Logo 档、日志和菜单的状态行如实显示。理由是它和"耐久等"不同类 —— 启动小窗本身可能停留好几分钟，用超时跳过它，等于在小窗还开着的时候就把**窗口加载档**套上去，而那正是这套分阶段想避免的事。代价是：若哪天游戏真把两个窗口都改名，`窗口加载`/`游戏中`两档就永远不会生效（而不是"用错档"）——菜单状态行看得见，不会骗人。
