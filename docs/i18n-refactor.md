...
> **引用说明（2026-09-19）**：本文件引用了若干**不随仓库发布**的文档 —— `docs\*-reverse.md`（各插件的逆向记录）与两份公测文档（`beta-audit-first-public-beta.md`、`beta-test-plan.md`）。它们仍在作者的工作区里，只是不再被 `git` 跟踪（见仓库根 `.gitignore`）；文中引用的行号照旧成立。
