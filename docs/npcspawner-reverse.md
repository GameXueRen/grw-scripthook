...
7. **每个落点先探地面再生成**（2026-09-16）。原件与框架的纯规划函数
   （`ShNpcPlanFormation`）都沿用玩家脚下的 Z，于是斜坡、台阶、桥面上
   会有 NPC 埋进地里；现在的生成路径在放每个 NPC 之前，用引擎自己的射线
   把落点放回地面（`ShGroundHeightFrom`，提示式探不到再 `ShGroundHeight`
   全高度扫一次），并抬高 0.5 m。探不到地面的那个点**跳过**——状态行的
   「未出现」会如实把它算进去，日志另有一行说明。性能代价：每次生成最多
   多 10 条射线（引擎自身约 10 万条/秒），且全部发生在插件的生成线程上。
...
- 种群管理器最初写的是**五个候选槽**（`4BACFA8` / `4B957A8` / `4B99BA8` / `4BBE3A8` /
  `4B8D7A8`），逐个做「可读 + 首字是镜像内虚表」检查，全不通过就**跳过并记账**。
  实测这五个**一个都不是**管理器：战役里它们在玩的那个读空，于是每次生成都跳过注册、
  完成行如实写 `NOT registered`。
  2026-09-16 改用**注册调用本身的参数**定死：引擎有 **5 处**调用注册函数，每处都是
  `48 8B 0D <disp32>`（`mov rcx,[rip+disp32]`）取管理器、`48 89 C2`（`mov rdx,rax`）
  带 spec、再无参调用。逐个解码那 5 条指令：

  | 调用点 | 取到的槽 |
  | --- | --- |
  | `775B74D` / `9202385` / `9EAB00D` / `13053BBB` | **`4B98FA8`** ✓（四点一致）|
  | `718E325` | `4B9A7A8`（同一区块，另一模式）|

  四点独立一致 ⇒ **`RVA_POP_MGR = 0x4B98FA8`**（它就在退役的 context 槽
  `4B98F98` 之后 `0x10`，同一区块）。**已实测**：完成行由 `NOT registered`
  变为 `registered`，且不再崩。真正把 NPC 放进世界的是 `COMMIT`，那条一直有效。
...
还有一处不是依赖而是**字段来源**：阵营与职业（`camp`/`job`，曾写进 spec 的
`+0x2D0` / `+0x2D4`）取自 `RVA_CONTEXT`。`4B98F98` 这个**槽地址本身**大概没错
（整个镜像只有一处代码「取一个 context，再读 `+0x1B4` 与 `+0x1B8`」，解出的位移
`+0x90` 与紧邻的 `RES_GLOBAL`、`SKILL_GLOBAL` 完全一致），**错的是字段的含义**：
那两处读出来的值是**浮点位型**（`0.4f` / `5.0f`），根本不是阵营号和职业号。
于是两件事被同时推翻：

1. `spec+0x2D0` / `+0x2D4` **不是两个 32 位 id**。镜像里写这两个偏移的两处代码
   都是 `48 89 91 D0 02 00 00`（`mov [rcx+0x2D0], rdx`）——**存的是 64 位指针**。
   把两个浮点位型塞进指针字段 ⇒ 注册函数解引用它就崩，这就是
   `scripthook_crash.log` 里第二次崩溃（寄存器里正好躺着 `-1` / `0.4f` / `5.0f`）。
2. 删掉这两次写入后 **NPC 立刻会动了**（此前「生成得出来、呆立不动」的根因就是它），
   也就是说不写是对的：那个字段由引擎自己按流程装填。`RVA_CONTEXT` 现已**退役**，
   没有任何代码读它。
...
| 那一版 | 现象 | 记下的结论 |
| --- | --- | --- |
| 陈旧函数常量 + 五候选槽 + 写 `+0x2D0` | 召唤不出来；随后**崩溃** | 常量已重钉（见 11.1–11.3），`+0x2D0` 的成因见 11.5 |
| 恢复严格检查 + **停写** `+0x2D0` | **不崩 ✓ / 会动了 ✓ / 但不攻击 ✗** | 不写是对的：那个字段是引擎填的 64 位指针 |
| 注册槽改为解码所得 `4B98FA8` | **不崩 ✓ / `registered` ✓** | 槽的解码正确（11.4 已记） |

对应日志：

```
npc: spawn id … -> spec …, NOT registered                     ← 五个候选槽全不中
npc: spawn id … -> spec … (mgr …, pop …), registered           ← 4B98FA8 生效 ✓
```

**搁置时的状态**：生成 ✓、移动 ✓、注册 ✓、反召唤 ✓ 都通了；**唯一没通的是
「不主动攻击」**。下一步的抓手已备好，尚未展开：

- 引擎在提交前会写 `spec+0x14C`，这一句我们从未做过：
  `9909F97: mov [rbx+14C], ebp`（`rbx` = spec，`ebp` = 其所在函数的第 3 个参数），
  紧接着 `9909FA3: call COMMIT`。要定死取值，需再解开调用它的上层函数。
- 另一种可能是敌我关系不在 spec 上，而在**生成请求/区域**那一层：引擎自己的
  生成点会带一个上下文对象，这需要从 `SPAWN`（跳板 `7D9A40`）的调用点继续往上看。
- 顺带遗留：`RVA_CONTEXT`（`4B98F98`）已退役未删；`4B90208` 这个旧值**从来不是**
  任何槽，代码里已无引用。

按需求，`NPCSpawner` 已**移出公测集**（`build_msvc.ps1` 的 `$betaSet`），源码与构建
条目留在树里；本条线的其余工作就此搁置。
