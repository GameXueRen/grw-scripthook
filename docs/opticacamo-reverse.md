...
- **菜单重做**：原版只有一项四档 `0.0/0.25/0.50/0.75`，而且**总是**写因子（没有关闭的办法）；
  ...
  - **状态行只在菜单打开时推送**（`ShMenuIsOpen()`）：菜单没打开时一个字节都不写进菜单模型，
    期间发生的状态变化只记一个"待推送"标志，菜单一打开立刻显示当前值。框架的公共 API
    目前只能回答"菜单是否打开"，回答不了"当前是不是本插件那一页" —— 需要更严格的话，
    得给框架加一个 `ShMenuIsShowing(menu)` 之类的查询（本轮按用户"框架零改动"的决定没加）；
