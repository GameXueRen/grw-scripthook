...
static const char *DEFAULT_CONFIG =
    ...
    "; efficiency mode (EcoQoS, the switch Task Manager shows) while they\n"
    "; last, which lets the scheduler run the process slower and on the\n"
    "; efficiency cores; 0 (the default) leaves the class alone.\n"
    ...
    "; LowQoS - the switch holds the LOW priority class instead: a loading\n"
    "; screen yielding the machine is the same intent, said with what is\n"
    "; there. One log line and the CPU page both say when that happens.\n"
    "; Either way the play stage puts it back, and a state the process\n"
    "; arrived with is never touched at all.\n"
    "cpu_eco_boot=0\n"
    ...
    "zh_cn = 简体中文\n"
    "en = English\n"
    "; The ScriptHook settings page, its one [loader] row and its sub pages.\n"
    "ScriptHook settings = ScriptHook 设置\n"
    "Plugin switches = 各插件开关\n"
    "CPU scheduling = CPU 调度控制\n"
    "Load plugins (master switch) = 加载插件（总开关）\n"
    "Boot cores = 1-启动Logo窗口加载\n"
    "Loading cores = 2-游戏主窗口加载\n"
    "Play cores = 3-游戏中\n"
    "Leave alone = 不干涉\n"
    "All cores = 全部核心(强制)\n"
    "SMT off = 禁超线程\n"
    ...
    "Efficiency mode: not available on this system - the loading stages hold the low priority instead. = 效率模式：本机不支持，加载阶段改用「低」优先级代替。\n"
    "Efficiency mode: the call failed - the loading stages hold the low priority instead. = 效率模式：设置失败，加载阶段改用「低」优先级代替。\n"
    "Processor set and priority for each start up stage - changes need a restart. = 按启动阶段设置处理器集合与优先级，改动重启生效。\n"
    "Now: %s - cores %s - priority %s = 当前：%s · 核心控制 %s · 优先级 %s\n"
    "\n"
    "[zh_cn.Chaos]\n"
    "Enabled = 混沌开关\n"
