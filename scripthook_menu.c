 ...
 * Added for the field report of 2026-09-17, where three menu complaints
 * arrived together: a row showing the page's own hint text, the language
 * row reading as a page name, and every value on one page reading the
 * same number. All three look like the model and the view disagreeing
 * about what a row is - and nothing in the shipped build could say which
 * side was wrong, because the model is only ever visible through the
 * view that is built from it.
 *
 * So for a bounded stretch of a session this writes both, once a second
 * while the menu is open: the model (per item: label, kind, value, option
 * count and the option in force) and the view captured from it. It also
 * checks, every frame, for the exact two shapes the report describes - a
 * row wearing text that belongs to the page, and a page whose rows all
 * show one value while its model holds several - and writes a named line
 * the first time it sees either. Those checks are cheap string compares
 * on at most twelve rows, and they turn the next session's log into the
 * answer instead of another round of guessing.
 *
 * Bounded twice over: one dump a second, and MENU_DIAG_DUMPS of them for
 * the whole session. A session that never opens the menu pays nothing,
 * and a long one stops on its own with a line that says it did.
 */
#define MENU_DIAG_DUMPS 50
#define MENU_DIAG_SNAP  12
...
typedef struct {
    ...
    char     hint[96];
    char     status[96];
    char     label[MENU_DIAG_SNAP][48];
    int      kind[MENU_DIAG_SNAP];
    int      value[MENU_DIAG_SNAP];
    int      nopts[MENU_DIAG_SNAP];
    char     opt[MENU_DIAG_SNAP][24];
} MenuDiagSnap;

static MenuDiagSnap g_diagSnap;

/* Called with the lock held, right after the view has been filled, so the
 * two are the same instant of the same menu. */
static void MenuDiagTake(const Menu *m) {
    int i, n;

    memset(&g_diagSnap, 0, sizeof(g_diagSnap));
    if (!m) return;
    g_diagSnap.id = g_current;
    g_diagSnap.count = m->count;
    SafeCopy(g_diagSnap.title, sizeof(g_diagSnap.title), m->title);
    SafeCopy(g_diagSnap.hint, sizeof(g_diagSnap.hint), m->hint);
    SafeCopy(g_diagSnap.status, sizeof(g_diagSnap.status), m->status);
    n = m->count < MENU_DIAG_SNAP ? m->count : MENU_DIAG_SNAP;
    for (i = 0; i < n; i++) {
        const Item *it = &m->items[i];

        SafeCopy(g_diagSnap.label[i], sizeof(g_diagSnap.label[i]), it->label);
        g_diagSnap.kind[i] = it->kind;
        g_diagSnap.value[i] = it->value;
        g_diagSnap.nopts[i] = it->nopts;
        if (it->kind == IT_LIST && it->nopts > 0)
            SafeCopy(g_diagSnap.opt[i], sizeof(g_diagSnap.opt[i]),
                     it->opts[it->value]);
    }
}

/* Called with the lock released: the compares are cheap, the file write is
 * not, and nothing here needs to hold the menu. */
static void MenuDiagMaybe(const ShMenuView *v) {
    static DWORD lastDump;
    static int dumps;
    static int saidRowText, saidOneValue, saidStop;
    DWORD now = GetTickCount();
    int i, j, distinct = 0, allSame = 1;

    if (v->rows < 1 && !v->title[0]) return;

    /* Shape one: a row whose label, or whose value, is text that belongs
     * to the page. Neither can come from the item - a label is copied in
     * when the item is made and a value is formatted from that item - so
     * seeing either means the two sides are not the same menu. The value
     * case is the one the field report shows: an hour row reading out the
     * status line, which is the page's text and not any row's. */
    if (!saidRowText) {
        for (i = 0; i < v->rows; i++) {
            const char *n2 = v->row[i].name;
            const char *val = v->row[i].value;

            if (n2[0]) {
                /* Equality, not containment: a row's label can legitimately
                 * appear inside the hint ("Plugin switches" is in the hint
                 * about plugin switches), which is what made this fire once
                 * on 2026-09-17 mid-language-switch - a false positive that
                 * costs the next session a look. A row wearing the page's
                 * text is the shape that matters, and it is equality. */
                if ((v->title[0] && strcmp(v->title, n2) == 0) ||
                    (v->status[0] && strcmp(v->status, n2) == 0) ||
                    (v->hint[0] && strcmp(v->hint, n2) == 0)) {
                    saidRowText = 1;
                    Log("menu diag: ANOMALY row %d carries page text as its "
                        "label: row=\"%s\" title=\"%s\"", i, n2, v->title);
                    break;
                }
            }
            if (val[0] && v->status[0] &&
                (strcmp(val, v->status) == 0 || strstr(v->status, val))) {
                saidRowText = 1;
                Log("menu diag: ANOMALY row %d carries the status line as its "
                    "value: row=\"%s\" value=\"%s\"", i, n2, val);
                break;
            }
        }
    }

    /* Shape two: the rows all show one value while the model behind them
     * holds several. Toggles are exempt - a page of switches reading
     * "[on]" everywhere is what it is supposed to do. */
    if (!saidOneValue && v->rows >= 3) {
        int model = 0;

        for (i = 0; i < g_diagSnap.count && i < MENU_DIAG_SNAP; i++)
            if (g_diagSnap.kind[i] == IT_LIST ||
                g_diagSnap.kind[i] == IT_NUMBER) model++;
        for (i = 1; i < v->rows; i++)
            if (strcmp(v->row[0].value, v->row[i].value) != 0) allSame = 0;
        for (i = 0; i < v->rows; i++) {
            for (j = 0; j < i; j++)
                if (strcmp(v->row[j].value, v->row[i].value) == 0) break;
            if (j == i) distinct++;
        }
        if (allSame && model >= 3 && v->row[0].value[0]) {
            saidOneValue = 1;
            Log("menu diag: ANOMALY %d rows share one value: \"%s\" "
                "(%d model rows are lists or numbers)",
                v->rows, v->row[0].value, model);
        } else if (distinct < model && distinct == 1 && model >= 3) {
            saidOneValue = 1;
            Log("menu diag: ANOMALY %d rows hold %d distinct values for "
                "%d model rows", v->rows, distinct, model);
        }
    }

    if (dumps >= MENU_DIAG_DUMPS) {
        if (!saidStop) {
            saidStop = 1;
            Log("menu diag: stopping after %d dumps", dumps);
        }
        return;
    }
    ...
    Log("menu diag: view id=%u rows=%d sel=%d root=%d title=\"%s\" "
        "hint=\"%s\" status=\"%s\" footer=\"%s\"",
        g_diagSnap.id, v->rows, v->sel, v->isRoot, v->title, v->hint,
        v->status, v->footer);
    for (i = 0; i < g_diagSnap.count && i < MENU_DIAG_SNAP; i++)
        Log("menu diag:   item[%d] label=\"%s\" kind=%d value=%d nopts=%d "
            "opt=\"%s\"", i, g_diagSnap.label[i], g_diagSnap.kind[i],
            g_diagSnap.value[i], g_diagSnap.nopts[i], g_diagSnap.opt[i]);
    for (i = 0; i < v->rows; i++)
        Log("menu diag:   row[%d] name=\"%s\" value=\"%s\" sel=%d",
            i, v->row[i].name, v->row[i].value, v->row[i].selected);
}
...
void ShMenuCaptureView(ShMenuView *v) {
    ...
    /* The model as it stood for this capture - same lock, same instant -
     * so the log below can compare the two sides of it. */
    MenuDiagTake(m);
    Unlock();
    MenuDiagMaybe(v);
}
